...
+ (BOOL) print2DBarCode:(int)type para1:(int)v para2:(int)r para3:(int)k content:(NSData*)data
{
    ...
    NSData *cmd2 = [[NSData alloc] initWithBytes:byte length:10];
    if(![self printBin:cmd2]) return NO;
    if(![self printBin:data]) return NO;
    return YES;
...
+ (BOOL) print1DBarCode:(int)type width:(int)w height:(int)h txtpositon:(int)positon content:(NSData*)data;
{
    ...
    if (type>=0 && type<=6)
    {
        ...
        cmd1 = [[NSData alloc] initWithBytes:byte length:(12+[data length])];
    }
    else if (type>=65 && type<=73)
    {
        ...
        cmd1 = [[NSData alloc] initWithBytes:byte length:(13+t)];
    }
    else return false;
    NSLog(@"cmd=%@",cmd1);
    if(![self printBin:cmd1]) return NO;
    return YES;
...
+ (BOOL) print128BarCode: (int)w height2:(int)h txtpositon2:(int)positon content0:(NSData*)data content2:(Byte *)dataBytes length2:(int)dataBytesLen;
{
    ...
    cmd1 = [[NSData alloc] initWithBytes:byte length:idx];
...
+ (BOOL) printBitMap:(int)mode bitmap:(NSData*)bm
{
    ...
    cmd1 = [[NSData alloc] initWithBytes:byte length:5+[bm length]];
    //NSLog(@"cmd=%@",cmd);
    if(![self printBin:cmd1]) return NO;
    return YES;
}
+ (BOOL) setLineHeight:(int)n
{
    ...
    byte[1]=0x33;
    byte[2]=n;
    cmd1 = [[NSData alloc] initWithBytes:byte length:3];
    if(![self printBin:cmd1]) return NO;
    return YES;
...
+ (BOOL) restoreDefaultLineHeight
{
    ...
    byte[0]=0x1b;
    byte[1]=0x32;
    cmd1 = [[NSData alloc] initWithBytes:byte length:2];
    if(![self printBin:cmd1]) return NO;
    return YES;
...
+ (BOOL) cutPaper:(int) mode feed_distance:(int) dis
{
    ...
    {
    	byte[3]=dis;
    	cmd1 = [[NSData alloc] initWithBytes:byte length:4];
    }
    if(![self printBin:cmd1]) return NO;
    return YES;
}
...
+ (void) print:(int) horizontal skipNum:(int) skip
{
    ...
    cmdaaa=DLE_EOT_5;      // qzfeng 2015/12/23
...
+ (void) feed
{
    ...
    NSData * cmd1;
    byte[0]=0x0c;
   	cmd1 = [[NSData alloc] initWithBytes:byte length:1];
    if(![self printBin:cmd1])
    {
